...
var HookFormContext = Cn.createContext(null);
var useFormContext = () => Cn.useContext(HookFormContext);
var FormProvider = (props) => {
  const { children, ...data } = props;
  return Cn.createElement(HookFormContext.Provider, { value: data }, children);
};
...
function useFormState(props) {
  ...
  const [formState, updateFormState] = Cn.useState(control._formState);
  const _mounted = Cn.useRef(true);
  const _localProxyFormState = Cn.useRef({
    isDirty: false,
    isLoading: false,
    dirtyFields: false,
    touchedFields: false,
...
function useWatch(props) {
  ...
  const [value, updateValue] = Cn.useState(control._getWatch(name, defaultValue));
  Cn.useEffect(() => control._removeUnmounted());
  return value;
}
...
function useController(props) {
  ...
  const _registerProps = Cn.useRef(control.register(name, {
    ...props.rules,
    value
  }));
  ...
  return {
    field: {
      ...
      value,
      disabled,
      onChange: Cn.useCallback((event) => _registerProps.current.onChange({
        target: {
      ...
      onBlur: Cn.useCallback(() => _registerProps.current.onBlur({
        target: {
...
function Form(props) {
  const methods = useFormContext();
  const [mounted, setMounted] = Cn.useState(false);
  ...
  return render ? Cn.createElement(Cn.Fragment, null, render({
    submit
  })) : Cn.createElement("form", { noValidate: mounted, action, method, encType, onSubmit: submit, ...rest }, children);
}
...
function useFieldArray(props) {
  ...
  const [fields, setFields] = Cn.useState(control._getFieldArray(name));
  const ids = Cn.useRef(control._getFieldArray(name).map(generateId));
  const _fieldIds = Cn.useRef(fields);
  const _name = Cn.useRef(name);
  const _actioned = Cn.useRef(false);
  _name.current = name;
  ...
  const updateValues = Cn.useCallback((updatedFieldArrayValues) => {
  ...
  return {
    swap: Cn.useCallback(swap, [updateValues, name, control]),
    move: Cn.useCallback(move, [updateValues, name, control]),
    prepend: Cn.useCallback(prepend$1, [updateValues, name, control]),
    append: Cn.useCallback(append$1, [updateValues, name, control]),
    remove: Cn.useCallback(remove, [updateValues, name, control]),
    insert: Cn.useCallback(insert$1, [updateValues, name, control]),
    update: Cn.useCallback(update, [updateValues, name, control]),
    replace: Cn.useCallback(replace, [updateValues, name, control]),
    fields: Cn.useMemo(() => fields.map((field, index) => ({
      ...field,
