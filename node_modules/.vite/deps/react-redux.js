...
function getContext() {
  ...
  let realContext = contextMap.get(G);
  if (!realContext) {
    realContext = G(null);
    if (true) {
      realContext.displayName = "ReactRedux";
    }
    contextMap.set(G, realContext);
  }
  return realContext;
}
...
var useIsomorphicLayoutEffect = canUseDOM ? y2 : p;

// node_modules/react-redux/es/utils/shallowEqual.js
function is(x3, y3) {
  if (x3 === y3) {
    return x3 !== 0 || y3 !== 0 || 1 / x3 === 1 / y3;
  } else {
    return x3 !== x3 && y3 !== y3;
  }
}
...
function connect(mapStateToProps, mapDispatchToProps, mergeProps, {
  ...
  const wrapWithConnect = (WrappedComponent) => {
    ...
    function ConnectFunction(props) {
      const [propsContext, reactReduxForwardedRef, wrapperProps] = F(() => {
        const {
      ...
      const ContextToUse = F(() => {
        return propsContext && propsContext.Consumer && // @ts-ignore
        (0, import_react_is.isContextConsumer)(y(propsContext.Consumer, null)) ? propsContext : Context;
      }, [propsContext, Context]);
      const contextValue = q(ContextToUse);
      ...
      const childPropsSelector = F(() => {
        return finalPropsSelectorFactory(store.dispatch, selectorFactoryOptions);
      }, [store]);
      const [subscription, notifyNestedSubs] = F(() => {
        if (!shouldHandleStateChanges)
      ...
      const overriddenContextValue = F(() => {
      ...
      const lastChildProps = _();
      const lastWrapperProps = _(wrapperProps);
      const childPropsFromStoreUpdate = _();
      const renderIsScheduled = _(false);
      const isProcessingDispatch = _(false);
      const isMounted = _(false);
      const latestSubscriptionCallbackError = _();
      ...
      const actualChildPropsSelector = F(() => {
        const selector = () => {
...
`;
      ...
      const renderedWrappedComponent = F(() => {
        return (
          // @ts-ignore
          y(WrappedComponent, _extends({}, actualChildProps, {
            ref: reactReduxForwardedRef
          }))
        );
      }, [reactReduxForwardedRef, WrappedComponent, actualChildProps]);
      const renderedChild = F(() => {
        if (shouldHandleStateChanges) {
          return y(ContextToUse.Provider, {
    ...
    const _Connect = x2(ConnectFunction);
    const Connect = _Connect;
    Connect.WrappedComponent = WrappedComponent;
    Connect.displayName = ConnectFunction.displayName = displayName;
    if (forwardRef) {
      const _forwarded = k(function forwardConnectRef(props, ref) {
        return y(Connect, _extends({}, props, {
...
function Provider({
  ...
  const previousState = F(() => store.getState(), [store]);
  useIsomorphicLayoutEffect(() => {
    const {
      subscription
    } = contextValue;
